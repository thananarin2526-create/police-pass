...
async function start(){
  ...
  app.get('/api/health',(req,res)=>res.json({ok:true,service:'POLICE PASS v12.2',version:'12.2.0'}));
  app.get('/api/ready',async(req,res)=>{
    try{const {q}=require('./db');await q('select 1 as ok');res.json({ok:true,database:'ready'})}
  ...
  app.listen(Number(process.env.PORT||3000),()=>console.log('POLICE PASS v12.2 running'));
}
